    ...
    for idx1 in range(1, N + 1):
        for idx2 in range(1, M + 1):
            for idx3 in range(1, X + 1):
                ...
                c += 1
                name = "A1[" + str(idx1) + "," + str(idx2) + "]ris_component1_" + str(idx3) + ":1"
                sources.append(["Name:=", name, "Magnitude:=", str(power) + "W", "Phase:=", "0deg"])

    sources.append(["Name:=", "IncPWave1", "Magnitude:=", "0V_per_meter", "Phase:=", "0deg"])
    return sources
...
def edit_sources_plane_wave():
    ...
    sources.append(["Name:=", "IncPWave1", "Magnitude:=", "1V_per_meter", "Phase:=", "0deg"])
    return sources
...
for idx in range(0, N * M * X):
    ...
    oModule2.ExportToFile("rEPhi", os.path.join(DIR, "rEPhi_" + str(idx) + ".csv"), False)
    oModule2.UpdateReports(["rETheta"])
    oModule2.ExportToFile("rETheta", os.path.join(DIR, "rETheta_" + str(idx) + ".csv"), False)

    f = 5.8
    dir_path = os.path.join(DIR, "f" + str(f))
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    export_near_field(os.path.join(dir_path, "near_field_cirlce" + str(idx) + ".nfd"), "circle", f)
    export_near_field(
        os.path.join(dir_path, "near_field_boresight" + str(idx) + ".nfd"),
        "boresight",
        f,
    )
    export_near_field(os.path.join(dir_path, "near_field_rect" + str(idx) + ".nfd"), "rect", f)
...
oModule2.UpdateReports(["rETheta"])
oModule2.ExportToFile("rETheta", os.path.join(DIR, "rETheta_" + "plane_0" + ".csv"), False)

f = 5.8
dir_path = os.path.join(DIR, "f" + str(f))
if not os.path.exists(dir_path):
    os.makedirs(dir_path)
export_near_field(os.path.join(dir_path, "near_field_plane_cirlce" + str(0) + ".nfd"), "circle", f)
export_near_field(
    os.path.join(dir_path, "near_field_plane_boresight" + str(0) + ".nfd"),
    "boresight",
    f,
)
export_near_field(os.path.join(dir_path, "near_field_plane_rect" + str(0) + ".nfd"), "rect", f)

for f in linspace(4.8, 6.8, 51):
    dir_path = os.path.join(DIR, "f" + str(f))
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
    export_near_field(os.path.join(dir_path, "near_field_plane_single" + str(0) + ".nfd"), "single", f)
